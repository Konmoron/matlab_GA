...
%% ---- 写入开始时间信息
    ...
    % ----- 写入时间信息        
        fprintf(fidTxtForObjFuncByBand, '* 本次仿真结束于%s\r\n', stopTimeStr);
%% ---- 写入仿真时间
    % ---- 得到仿真时间
        spendTime = etime(stopTime, startTime);
    % ---- 将仿真时间写入Data文件
        fprintf(fidTxtForObjFuncByBand, '* 本次仿真共用时%f秒\r\n', spendTime);
%% ---- 调用函数写入100个“*”
    write100Star(fidTxtForObjFuncByBand);
%% ---- 写入最佳个体
    fprintf(fidTxtForObjFuncByBand, '* 本次仿真得到的最佳个体是：pop = ');
    fprintf(fidTxtForObjFuncByBand, '%.0f ', pop);
    fprintf(fidTxtForObjFuncByBand, '\r\n');
%% ---- 写入最佳个体对应的适应度值
    fprintf(fidTxtForObjFuncByBand, '* 最佳个体的适应度值是：%.2f', fival);
    fprintf(fidTxtForObjFuncByBand, '\r\n');
%% ---- 调用函数写入100个“*”
    write100Star(fidTxtForObjFuncByBand);
    fprintf(fidTxtForObjFuncByBand, '\r\n');
