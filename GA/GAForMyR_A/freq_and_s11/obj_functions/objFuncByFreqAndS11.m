...
function scores = objFuncByFreqAndS11(pop)
%{ 
    ---- 注意 ----
    ---- 注意 ----
    ---- 注意 ----
            1.scores是染色体pop的适应度，由getScoresByFreqAndS11函数得到，也是适应度函数返回给遗传算法的值
...
%% ---- 定义tmpSwtichArray数组，并将pop赋值给tmpSwtichArray
    %{ 
        ...
        ---- 注意 ----
            ...
            tmpSwtichArray将被建模仿真脚本调用，用于判断开关的断开和闭合，详细信息可参见：myTestPop.m
        ---- 注意 ----
        ---- 注意 ----
        ---- 注意 ----
    %}
...
%% ---- 调用getScoresByFreqAndS11_V2函数，得到染色体pop的适应度值
    scores = getScoresByFreqAndS11_V3(freq, s11);
%% ---- 将所有信息写入文件
    writeDataForObjFuncByFreqAndS11;
%% ---- 输出当前染色体的信息
    fprintf('    经过计算，该染色体的工作频率是：%.2f，其对应的S11参数是：%.2f，适应度值是：%.3f\n', freq, s11, scores);
%% ---- 统计变量加1
    %{
        ---- 注意 ----
        ---- 注意 ----
